...
/**
  ...
  *
  * And transforms:
  *   FnToProduct: Function2[A, B, C] => Function1[ A :: B :: HNil , C]
  *   Generic: (VData[A], VData[B]) => VData[A] :: VData[B] :: HNil
  *   VProduct: VData[A] :: VData[B] :: HNil => A :: B :: HNil
  *
  *
  */
...
object GetGenericFToWork extends App {
  ...
  // TODO(nick.bradford) see Shapeless 4.3: Chaining dependent functions
  def applyWithVProd[InputTuple <: Product, Function, InputRepr <: HList, Result, UnboxedData <: HList]
    (p: InputTuple)
    (f: Function)
    (implicit
